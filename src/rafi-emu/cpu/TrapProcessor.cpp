...
void TrapProcessor::ProcessException(const Trap& trap)
{
    ...
    if ((m_pCsr->ReadUInt32(csr_addr_t::medeleg) & delegMask) != 0)
    {
        nextPrivilegeLevel = PrivilegeLevel::Supervisor;
        if ((m_pCsr->ReadUInt32(csr_addr_t::sedeleg) & delegMask) != 0)
        {
...
void TrapProcessor::ProcessTrapReturn(PrivilegeLevel level)
{
    ...
    case PrivilegeLevel::Machine:
        status = xstatus_t(m_pCsr->ReadUInt32(csr_addr_t::mstatus));
        pc = m_pCsr->ReadUInt32(csr_addr_t::mepc);
        ...
        m_pCsr->WriteUInt32(csr_addr_t::mstatus, status);
        m_pCsr->SetProgramCounter(pc);
        break;
    case PrivilegeLevel::Supervisor:
        status = xstatus_t(m_pCsr->ReadUInt32(csr_addr_t::sstatus));
        pc = m_pCsr->ReadUInt32(csr_addr_t::sepc);
        ...
        m_pCsr->WriteUInt32(csr_addr_t::sstatus, status);
        m_pCsr->SetProgramCounter(pc);
        break;
    default:
        RAFI_EMU_NOT_IMPLEMENTED();
...
void TrapProcessor::ProcessTrapEnter(bool isInterrupt, uint32_t exceptionCode, uint64_t trapValue, vaddr_t pc, PrivilegeLevel nextPrivilegeLevel)
{
    ...
    {
    case PrivilegeLevel::Machine:
        status = m_pCsr->ReadUInt32(csr_addr_t::mstatus);
        ...
        m_pCsr->WriteUInt32(csr_addr_t::mstatus, status);
        m_pCsr->WriteUInt32(csr_addr_t::mcause, cause);
        m_pCsr->WriteUInt32(csr_addr_t::mepc, static_cast<uint32_t>(pc));
        m_pCsr->WriteUInt32(csr_addr_t::mtval, static_cast<uint32_t>(trapValue));

        trapVector = xtvec_t(m_pCsr->ReadUInt32(csr_addr_t::mtvec));
        break;
    case PrivilegeLevel::Supervisor:
        status = m_pCsr->ReadUInt32(csr_addr_t::sstatus);
        ...
        m_pCsr->WriteUInt32(csr_addr_t::sstatus, status);
        m_pCsr->WriteUInt32(csr_addr_t::scause, cause);
        m_pCsr->WriteUInt32(csr_addr_t::sepc, static_cast<uint32_t>(pc));
        m_pCsr->WriteUInt32(csr_addr_t::stval, static_cast<uint32_t>(trapValue));

        trapVector = xtvec_t(m_pCsr->ReadUInt32(csr_addr_t::stvec));
        break;
    case PrivilegeLevel::User:
        status = m_pCsr->ReadUInt32(csr_addr_t::ustatus);

        status.SetMember<xstatus_t::UPIE>(status.GetMember<xstatus_t::UIE>());
        status.SetMember<xstatus_t::UIE>(0);

        m_pCsr->WriteUInt32(csr_addr_t::ustatus, status);
        m_pCsr->WriteUInt32(csr_addr_t::ucause, cause);
        m_pCsr->WriteUInt32(csr_addr_t::uepc, static_cast<uint32_t>(pc));
        m_pCsr->WriteUInt32(csr_addr_t::utval, static_cast<uint32_t>(trapValue));

        trapVector = xtvec_t(m_pCsr->ReadUInt32(csr_addr_t::utvec));
        break;
    default:
        RAFI_EMU_NOT_IMPLEMENTED();
    }
